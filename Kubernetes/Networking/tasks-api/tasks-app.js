...
// TO RESOLVE CORS (so we can access this through a browser (however, it's not a problem if we use POSTMAN))
...
const extractAndVerifyToken = async (headers) => {
  ...
  // Using ENV Variables
  // const response = await axios.get(`http://{process.env.AUTH_SERVICE_ADDRESS}/verify-token/` + token);

  // Using Auto-Generated ENV
  const response = await axios.get(`http://{process.env.SERVICE_AUTH_SERVICE_HOST}/verify-token/` + token);
